...
const ShiftedBusinessForm: React.FC<ShiftedBusinessFormProps> = ({ caseData }) => {
  ...
  const validationConfig = useMemo(() => {
    return new ValidationConfigBuilder<ShiftedBusinessReportData>(
      MIN_IMAGES,
      [
    ...
    // When office is opened, additional fields are required
    .addConditionalFields(
      'officeStatus',
      OfficeStatusOffice.Opened,
      [
    ...
    // String field validations for conditional fields
    .addStringField('companyNamePlateStatus', SightStatus.Sighted, 'nameOnBoard')
    .addStringField('finalStatus', FinalStatusShiftedBusiness.Hold, 'holdReason')
    // Custom validation for company name and period when premises is not vacant
    .addCustomValidation((report) => {
      if (report.officeStatus === OfficeStatusOffice.Opened &&
          report.premisesStatus !== PremisesStatusBusiness.Vacant) {
        return (report.currentCompanyName !== null && report.currentCompanyName !== undefined && report.currentCompanyName !== '') &&
               (report.currentCompanyPeriod !== null && report.currentCompanyPeriod !== undefined && report.currentCompanyPeriod !== '');
      }
      return true;
    })
    .build();
