...
const ResidenceForm: React.FC<ResidenceFormProps> = ({ caseData }) => {
  ...
  const validationConfig = useMemo(() => {
    return new ValidationConfigBuilder<ResidenceReportData>(
      MIN_IMAGES,
      [
        'addressLocatable', 'addressRating', 'houseStatus',
        'stayingPeriod', 'stayingStatus', 'approxArea', 'documentShownStatus',
        'tpcMetPerson1', 'tpcName1', 'tpcConfirmation1', 'tpcMetPerson2', 'tpcName2', 'tpcConfirmation2',
    ...
    // When house is opened, additional fields are required
    .addConditionalFields(
      'houseStatus',
      HouseStatus.Opened,
      ['metPersonName', 'metPersonRelation', 'totalFamilyMembers', 'totalEarning', 'applicantDob', 'workingStatus']
    )
    ...
    // Custom validation for company name when working status is not HouseWife
    .addCustomValidation((report) => {
      if (report.houseStatus === HouseStatus.Opened &&
          report.workingStatus !== WorkingStatus.HouseWife) {
        return report.companyName && report.companyName.trim() !== '';
      }
      return true;
    })
    .build();
