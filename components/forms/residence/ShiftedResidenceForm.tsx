...
const ShiftedResidenceForm: React.FC<ShiftedResidenceFormProps> = ({ caseData }) => {
  ...
  const validationConfig = useMemo(() => {
    return new ValidationConfigBuilder<ShiftedResidenceReportData>(
      MIN_IMAGES,
      [
    ...
    // When room is opened, additional fields are required
    .addConditionalFields(
      'roomStatus',
      RoomStatusShifted.Opened,
      ['metPersonName', 'metPersonStatus', 'shiftedPeriod', 'tpcMetPerson1', 'tpcName1', 'tpcMetPerson2', 'tpcName2']
    )
    // When room is closed, premises status is required
    .addConditionalFields(
      'roomStatus',
      RoomStatusShifted.Closed,
      ['premisesStatus']
    )
    ...
    .addStringField('finalStatus', FinalStatusShifted.Hold, 'holdReason')
    .build();
  }, []);
