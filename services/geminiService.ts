...
const ai = new GoogleGenAI({ apiKey: environment.getApiKey() });
...
export const summarizeText = async (text: string): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction:
          'You are an expert at creating concise, actionable summaries for field service technicians.',
        temperature: 0.5,
...
const formatReportForSummary = (caseData: Case): string => {
  ...
  reportContent += `Outcome: ${
    caseData.verificationOutcome || 'Not specified'
  }\n`;
...
export const summarizeReport = async (caseData: Case): Promise<string> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction:
          'You are an AI assistant that creates clear, brief summaries of field verification reports for case managers.',
        temperature: 0.3,
      },
    });
